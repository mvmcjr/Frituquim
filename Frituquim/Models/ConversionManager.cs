...
{
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(ShouldBeVisible))]
    ...
    [ObservableProperty] private string? _averageSpeed;
    ...
    private readonly Subject<(FileConversionTracker Tracker, double? Speed, TimeSpan? CurrentTime)> _progressSubject = new();
        ...
        int maxParallelism = 3)
    {
        ...
        await trackers.ToAsyncProcessorBuilder()
            .ForEachAsync(async tracker => await ProcessFile(tracker, conversionType, conversionHardware, cancellationToken))
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                await _dispatcher.InvokeAsync(() => tracker.SetDuration(duration.Value));
            }
        ...
        catch (OperationCanceledException)
        {
            ...
            if (File.Exists(tracker.OutputPath))
            {
                try { File.Delete(tracker.OutputPath); } catch { }
            }
        }
        catch (Exception ex)
        {
    ...
    private void HandleProgressUpdate(FileConversionTracker tracker, string progressLine)
    {
        ...
        if (hasUpdate)
        {
            _progressSubject.OnNext((tracker, speed, currentTime));
        }
    }
        ...
        if (activeFilesWithETA.Count > 0)
        {
            ...
            else
            {
                EstimatedTimeRemaining = maxActiveETA.TotalSeconds > 0
                    ? (maxActiveETA.TotalHours >= 1 ? maxActiveETA.ToString(@"hh\:mm\:ss") : maxActiveETA.ToString(@"mm\:ss"))
                    : null;
            }
        }
