...
public partial class ExtractionViewModel(ExtractionType extractionType, ISnackbarService snackbarService, ExecutionMessageMap messageMap) : FrituquimBasePageWithOutputDirectory
{
    ...
    private async Task Execute()
    {
        ...
        try
        {
            ...
            var downloadedFile = await YtdlpHelper.CreateYtdlpCommand(VideoPathOrUrl, downloadFilePath, extractionType == ExtractionType.Audio ? ["-x", "--audio-format", "mp3"] : [])
                .ExecuteAsync()
                .Task
